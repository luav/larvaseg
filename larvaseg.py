...
if __name__ == '__main__':
	...
	for wlc in args.input:
		for ifname in iglob(wlc):
			...
			out = cv.VideoWriter('outp-f.8.avi', cv.VideoWriter_fourcc(*'XVID'), fps, (width, height))
			if not cap:
				exit(1)
			...
			while cap.isOpened():
				...
				if not ret:
					...
					break
				cv.imshow(title + '#' + str(nframe), frame)
				...
				cv.imshow(title + '_thresh#' + str(nframe), thresh)
				# cv.imshow(title + '_gray#' + str(nframe), frame)
				...
				cv.imshow(title + '_opening#' + str(nframe), opening)
				# opening = thresh
				...
				cv.imshow(title + '_bg#' + str(nframe), sure_bg)
				# sure_bg = opening
				# Foreground identification
				dist_transform = cv.distanceTransform(opening, cv.DIST_L2, 5)  # 5 or 3
				ret, sure_fg = cv.threshold(dist_transform, 0.8 * dist_transform.max(), 255, 0)  # 0.7 [0.5 - deliv]]
				# Remaining areas
				sure_fg = np.uint8(sure_fg)
				unknown = cv.subtract(sure_bg, sure_fg)
				cv.imshow(title + '_unknown#' + str(nframe), unknown)
